...
from .module import ShortcutConv2d, DoubleConv, ChannelAttention, SpatialAttention
...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv):
        ...
        self.downchannel_layers = self.build_downchannel((512, 256, 128), (256, 128, 64))
    ...
    def build_downchannel(self, inplanes, planes):
        assert len(inplanes) == len(planes)
        downchannel_layers = []
        for inp, outp in zip(inplanes, planes):
            layer =DoubleConv(inp, outp)
            downchannel_layers.append(layer)
        return nn.Sequential(*downchannel_layers)
    ...
    def forward(self, x):
        ...
        for i, upsample_layer in enumerate(self.deconv_layers):
            ...
            if i < len(self.shortcut_layers):
                y4 = torch.cat([y4 ,feats[-i - 2]], dim = 1)
                y4 = self.downchannel_layers[i](y4)
        ret = {}
        for head in self.heads:
