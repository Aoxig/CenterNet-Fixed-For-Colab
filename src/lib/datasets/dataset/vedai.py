...
class VEDAI(data.Dataset):
    ...
    def __init__(self, opt, split):
        super(VEDAI, self).__init__()
        self.data_dir = os.path.join(opt.data_dir, 'vedai')
        self.img_dir = os.path.join(self.data_dir, 'images')
        _ann_name = {'train': 'train2017', 'val': 'test2017'}
        self.annot_path = os.path.join(
            self.data_dir, 'annotations',
            'vedai_{}.json').format(_ann_name[split])
        self.max_objs = 50
        self.class_name = ['__background__', "car", "truck", "pickup", "tractor", "camping car",
        ...
        print('==> initializing vedai {} data.'.format(_ann_name[split]))
